...
export const  authenticateToken = (req, res, next ) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decoded) => {
        if (err){
            ...
            return res.status(403).json({message: "Unauthorize", error: err.message});
        }
        // console.log("user token data: ",decoded);
        req.user = await User.findById(decoded.id, {permission: 1, isSuperAdmin:1, role:1}).populate('permission');
